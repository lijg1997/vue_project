                    ...
                    <div :class="{on:loginWay === 'message'}">
                        <section class="login_message">
                            ...
                            <button :disabled="!phoneNumberIsRight || times > 0" class="get_verification"
                                @click.prevent="getCode" :class="{highLight:phoneNumberIsRight}">
                        ...
                        <section>
                            ...
                            <section class="login_message">
                                <input type="text" maxlength="4" placeholder="验证码"
                                       v-model="captcha" name="captcha" v-validate="{required:true, regex: /^[a-z0-9]+$/i}">
                                <img class="get_verification" ref="captchaImg"
                                    :src="captchaImg" alt="captcha" @click="changeCaptcha">
    ...
    export default {
        ...
        methods:{
            ...
            async getCode(){
                ...
                const body = await this.$http.login.getCode({phone:this.phone});
                console.log(body);
                if(body.code === OK) toast.success('短信发送成功');
                else if(body.code === ERROR) toast.fail(body.msg)
            },
            async userLogin(){
            ...
            changeCaptcha(){
                this.$refs.captchaImg.src = this.captchaImg + new Date().getTime()
            }
        }
    }
</script>
